...
enum {
    ...
    kKeyAudioFromat       = 'afrt',
    kKeyAudioChLayout     = 'achl',
    ...
    kKeyFFmpegStream      = 'ffst',

    kKeyMediaBufferQueue = 'mbqu',

    kKeyTrackActive = 'tkat',

    kKeyIOSHVDAActive = 'iosh',

    kKeyAndroidHVDAActivie = 'andh',

    kKeyDownloadSpeed = 'dlds',

    kKeyDownloadPercent = 'dldp',

    kKeyStartTime = 'stti',

    kKeyMediaType = 'mdty',

    kKeyDoblyAudioCodecs = 'dbac',

    kKeyDTSAudioCodecs = 'dtsa',
...
public:
    NBMetaData();
    NBMetaData(const NBMetaData &from);
...
private:
    struct typed_data {
        ...
        typed_data(const NBMetaData::typed_data &);
        typed_data &operator=(const NBMetaData::typed_data &);
    ...
    NBMap<uint32_t, typed_data> mItems;

private:
    NBMetaData &operator=(const NBMetaData &);
};
